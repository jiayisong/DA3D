...
from mmyolo.models.layers import Cat
...
class BaseYOLONeck(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.top_down_fuse_layers = nn.ModuleList()
        for idx in range(len(in_channels) - 1, 0, -1):
            ...
            self.top_down_fuse_layers.append(self.build_fuse_layer(idx - 1))
        # build bottom-up blocks
        self.downsample_layers = nn.ModuleList()
        self.bottom_up_layers = nn.ModuleList()
        self.bottom_up_fuse_layers = nn.ModuleList()
        for idx in range(len(in_channels) - 1):
            ...
            self.bottom_up_fuse_layers.append(self.build_fuse_layer(idx))
        self.out_layers = nn.ModuleList()
        for idx in range(len(in_channels)):
            self.out_layers.append(self.build_out_layer(idx))

    def build_fuse_layer(self, idx: int):
        return Cat()
    ...
    def forward(self, inputs: List[torch.Tensor]) -> tuple:
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            if self.upsample_feats_cat_first:
                top_down_layer_inputs = self.top_down_fuse_layers[len(self.in_channels) - 1 - idx](
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            out = self.bottom_up_layers[idx](
                self.bottom_up_fuse_layers[idx]([downsample_feat, feat_high])
            )
            outs.append(out)
