...
def dw_conv3x3(in_channels, out_channels, module_name, postfix, stride=1, kernel_size=3, padding=1):
    ...
    return [
        ...
        ('{}_{}/pw_norm'.format(module_name, postfix), build_norm_layer(_NORM, out_channels)[1]),
...
def conv3x3(in_channels, out_channels, module_name, postfix, stride=1, groups=1, kernel_size=3, padding=1):
    ...
    return [
        ...
        (f"{module_name}_{postfix}/norm", build_norm_layer(_NORM, out_channels)[1]),
...
class VoVNet(BaseModule):
    def __init__(self, depth,
        ...
        stage_specs = _STAGE_SPECS[depth]
        ...
        self._out_features = out_indices

        # Stem module
        conv_type = dw_conv3x3 if depthwise else conv3x3
        stem = conv3x3(input_channels, stem_ch[0], "stem", "1", 2)
    ...
    def forward(self, x):
        outputs = []
        if 0 in self._out_features:
            outputs.append(x)
        x = self.stem(x)
        if 1 in self._out_features:
            outputs.append(x)
        i = 2
