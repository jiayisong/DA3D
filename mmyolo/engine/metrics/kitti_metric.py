...
from .functional import kitti_eval
...
class KittiMetric(BaseMetric):
    ...
    def __init__(self,
        ...
        allowed_metrics = [['bbox'], ['3d', 'bev'], ['bbox', '3d', 'bev', 'aos']]
        self.metrics = metric if isinstance(metric, list) else [metric, ]
        self.metrics = self.metric if isinstance(self.metrics[0], list) else [self.metrics, ]
        for metric in self.metrics:
            if metric not in allowed_metrics:
                raise KeyError("metric should be one of '[['img_bbox'], ['3d', 'bev'], ['bbox', '3d', 'bev', 'aos']]', "
                               'but got {metric}.')
    ...
    def kitti_evaluate(self,
        ...
        for name in results_dict:
            ...
            ap_result_str = self.print_more_beautifully(ap_dict_)
            print_log(f'Results of {name}:\n' + ap_result_str, logger=logger)

        return ap_dict

    def print_more_beautifully(self, ap_dict):
        cls = []
        difficult = []
        r11or40 = []
        bbox = []
        IOU = []
        for name in ap_dict.keys():
            name = name[6:].split('_')
            if name[0] not in cls:
                cls.append(name[0])
            if name[1] not in bbox:
                bbox.append(name[1])
            if name[2] not in r11or40:
                r11or40.append(name[2])
            if name[3] not in difficult:
                difficult.append(name[3])
            if len(name) == 5:
                if name[4] not in IOU:
                    IOU.append(name[4])
        line_title = ['Class', 'IOU', 'Difficult', ]
        for r in r11or40:
            for b in bbox:
                line_title.append(r + '_' + b)
        table_data = [line_title, ]
        for c in cls:
            for iou in IOU:
                for d in difficult:
                    line = [c, iou, d]
                    for r in r11or40:
                        for b in bbox:
                            line.append(f'{ap_dict["KITTI/" + c + "_" + b + "_" + r + "_" + d + "_" + iou]:.2f}')
    ...
    def format_results(self,
        ...
        for name in results[0]:
            ...
            if 'pred_instances' in name and '3d' in name and name[0] != '_' and len(results[0][name].keys()) > 0:
            ...
            elif name == 'pred_instances' and name[0] != '_' and len(results[0][name].keys()) > 0:
                net_outputs = [result[name] for result in results]
    ...
    def bbox2result_kitti(self,
        ...
        for idx, pred_dicts in enumerate(mmengine.track_iter_progress(net_outputs)):
            ...
            box_dict = self.convert_valid_bboxes(pred_dicts, info, 'bbox' in self.metrics)
            anno = {
                'name': [],
                'truncated': [],
                'occluded': [],
            ...
            if len(box_dict['bbox']) > 0:
                ...
                for i, (box, bbox, score, label) in enumerate(zip(box_preds, box_2d_preds, scores, label_preds)):
                    ...
                    if pred_box_type_3d == CameraInstance3DBoxes:
                        ...
                        anno['alpha'].append(-np.arctan2(-box_preds_lidar[i][1], box_preds_lidar[i][0]) + box[6])
    ...
    #             label_preds=np.zeros([0, 4]),
    #             sample_idx=sample_idx)
    def convert_valid_bboxes(self, box_dict: dict, info: dict, points_2d):
        """Convert the predicted boxes into valid ones.
        ...
        if points_2d:
            ...
            if valid_inds.sum() > 0:
                return dict(
                    ...
                    pred_box_type_3d=type(box_preds),
                    box3d_camera=box_preds_camera[valid_inds].tensor.numpy(),
                    box3d_lidar=box_preds_lidar[valid_inds].tensor.numpy() if isinstance(box_preds,
                                                                                         LiDARInstance3DBoxes) else None,
                    scores=scores[valid_inds].numpy(),
                    label_preds=labels[valid_inds].numpy(),
                    sample_idx=sample_idx)
            else:
            ...
            if isinstance(box_preds, LiDARInstance3DBoxes):
                lidar2cam = np.array(
                    info['images'][self.default_cam_key]['lidar2cam']).astype(
                    np.float32)
                box_preds_camera = box_preds.convert_to(Box3DMode.CAM, lidar2cam).tensor.numpy()
                box_preds_lidar = box_preds.tensor.numpy()
            elif isinstance(box_preds, CameraInstance3DBoxes):
                box_preds_camera = box_preds.tensor.numpy()
                box_preds_lidar = None
            return dict(
                bbox=np.ones([labels.shape[0], 4]) * (-1000),
                pred_box_type_3d=type(box_preds),
                box3d_camera=box_preds_camera,
                box3d_lidar=box_preds_lidar,
                scores=scores.numpy(),
