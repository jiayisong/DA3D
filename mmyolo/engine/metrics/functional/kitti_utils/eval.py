...
def get_f1(prec, recall):
    f = np.max(2 * prec * recall / np.clip(prec + recall, 1e-10, 10000), axis=-1, keepdims=False)
    return f * 100
...
def do_eval(gt_annos,
    ...
    if 'bbox' in eval_types:
        ...
        f1_bbox = get_f1(ret['precision'], ret['recall'])
    ...
    if 'bev' in eval_types:
        ...
        f1_bev = get_f1(ret['precision'], ret['recall'])
    ...
    if '3d' in eval_types:
        ...
        f1_3d = get_f1(ret['precision'], ret['recall'])
    return (mAP11_bbox, mAP11_bev, mAP11_3d, mAP11_aos, mAP40_bbox, mAP40_bev,
            mAP40_3d, mAP40_aos, f1_bbox, f1_bev, f1_3d)
...
def format_result(current_classes, class_to_name, min_overlaps, value, name):
    ret_dict = {}
        ...
        for i in range(min_overlaps.shape[0]):
            for idx in range(len(difficulty)):
                ...
                for v, n in zip(value, name):
                    if v is not None:
                        ret_dict[f'{prefix}_{n}_{postfix}'] = v[j, idx, i]
    ...
    if len(current_classes) > 1:
        ...
        for i in range(min_overlaps.shape[0]):
            for idx in range(len(difficulty)):
                ...
                for v, n in zip(value, name):
                    if v is not None:
                        ret_dict[f'{prefix}_{n}_{postfix}'] = np.mean(v[:, idx, i])
    return ret_dict
...
def kitti_eval(gt_annos,
    ...
    mAP11_bbox, mAP11_bev, mAP11_3d, mAP11_aos, mAP40_bbox, mAP40_bev, mAP40_3d, mAP40_aos, f1_bbox, f1_bev, f1_3d = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, eval_types)
    ret_dict = {}
    result = format_result(current_classes, class_to_name, min_overlaps, [mAP11_3d, mAP11_bev, mAP11_bbox, mAP11_aos],
                           ['3D_AP11', 'BEV_AP11', '2D_AP11', 'AOS_AP11'])
    ret_dict.update(result)
    result = format_result(current_classes, class_to_name, min_overlaps, [mAP40_3d, mAP40_bev, mAP40_bbox, mAP40_aos],
                           ['3D_AP40', 'BEV_AP40', '2D_AP40', 'AOS_AP40'])
    ret_dict.update(result)
    result = format_result(current_classes, class_to_name, min_overlaps, [f1_3d, f1_bev, f1_bbox],
                           ['3D_F1', 'BEV_F1', '2D_F1'])
    ret_dict.update(result)
    return '', ret_dict
